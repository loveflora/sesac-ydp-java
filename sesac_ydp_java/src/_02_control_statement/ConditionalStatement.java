...
//		System.out.println("name 값 확인 : " + name);
		...
		// BAD
//		if (name == "김새싹") {  // "김새싹" 올바르게 입력해도, else 절로 넘어감
...
//		
//		// GOOD
//		if (name.equals("김새싹")) {  // "김새싹" 올바르게 입력해도, else 절로 넘어감
...
//		}
		...
		// "문자열 리터럴(new 연산자로 문자열을 생성하지 않고, 큰 타옴표로 생성한 문자열)"일 경우, 
		...
		if(str1 == str2) {
			System.out.println("같은 참조를 가리킵니다.");  //-- 출력됨
		} else {
			System.out.println("다른 참조를 가리킵니다.");
		}
		
		if(str1.equals(str2)) {
			System.out.println("내용이 같다."); //-- 출력됨
		} else {
			System.out.println("내용이 다르다.");
		}
		
		// 문자열 "동적 할당" new String()을 이용해, 새로운 문자열 객체를 만드는 경우
		// new 연산자로 각각의 객체를 생성 (str3, str4)
		// str3, str4는 서로 다른 객체
		// '==' 연산자로 비교 시, false
		// .equals() 메소드로 비교 시, true
		...
		if(str3 == str4) {
			...
			System.out.println("다른 참조를 가리킵니다."); //-- 출력됨
		}
